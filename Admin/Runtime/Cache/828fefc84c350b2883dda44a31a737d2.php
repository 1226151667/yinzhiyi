...
<div id="body-wrapper">
 ...
 foreach($list as $row){ ?>
							<tr class="db">
								...
								<td><img style="height:80px" src="<?php echo $row['aFilePath'];?>"></td>
	...
	<div id="log" class="subjectTable">
		...
		<form method="post" enctype="multipart/form-data" action="<?php echo U('Index/column');?>">
...
$(function(){
	...
	$("button[class='search']").click(function(){
		...
		if(likeName==""){
			...
			window.location.href="column/name/"+ likeName;
		}
	});
	//删除
	$("button[class='delete']").click(function(){
		...
		if(del == ""){
			...
			window.location.href="column/del/"+ del;
		}
	});
	//查看全部
	$("button[class='all']").click(function(){
		window.location.href="<?php echo U('Index/column');?>";
	});
	//添加or修改
