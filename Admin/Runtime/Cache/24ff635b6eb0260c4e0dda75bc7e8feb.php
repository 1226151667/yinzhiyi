...
<!-- 					<div class="tbTop" name="Index">ipban</div>
 ...
 foreach($list as $row){ ?>
	...
	<div id="log" class="subjectTable">
		...
		<form method="post" action="<?php echo U('Index/ipBanTable');?>">
...
$(function(){
	...
	$("button[class='all']").click(function(){
		window.location.href="<?php echo U('Index/ipBanTable');?>";
	});
	//添加or修改
	$("button[name='sub']").click(function(){
		...
		var manageIp = $("input[name='manageIp']").val();
		if(ip=="" || nickname=="" || group=="" || manageIp==""){
			alert("所有带*号项都必填");
		}else{
			$("form").submit();
		}
	...
	edit.dblclick(function(){
		...
		$.get(
			'<?php echo U("Index/ipBanTable");?>',
			{id:id},
			function(data){
				$(".log_t span").text("修改页");
				$("input[name='ip']").val(data.ip);
				$("input[name='nickname']").val(data.nickname);
				$("input[name='manageIp']").val(data.manageIp);
